...
(function () {
  ...
  const wzNav        = document.querySelector('#view-vorbereiten .wz-nav');
  ...
  function showStep(n, dir) {
    ...
    const top = progressWrap.getBoundingClientRect().top + window.pageYOffset - 76;
  ...
  function buildPayload() {
    ...
    return {
      meta: {
        quelle: "Online-Scheidung Formular (Mobile)",
  ...
  async function submit() {
    ...
    try {
      ...
      const top = submitError.getBoundingClientRect().top + window.pageYOffset - 120;
      window.scrollTo({ top: Math.max(0, top), behavior: __RM ? 'auto' : 'smooth' });
    }
  }
  ...
  function onSuccess() {
    ...
    document.body.classList.remove('has-wznav');
    successEl.classList.add('show');
    const top = successEl.getBoundingClientRect().top + window.pageYOffset - 90;
    window.scrollTo({ top: Math.max(0, top), behavior: __RM ? 'auto' : 'smooth' });
  }
...
(function(){
  function setActiveView(name){
    ...
    document.body.classList.toggle('has-wznav', name!=='start');
    document.querySelectorAll('.app-tab[data-tab]').forEach(function(t){
...
(function(){
  'use strict';
  /* Eignungs-Quiz */
  var quiz=document.getElementById('suit-quiz');
  if(quiz){
    var ans={};
    quiz.querySelectorAll('.sq-q').forEach(function(q){
      q.querySelectorAll('button').forEach(function(b){
        b.addEventListener('click',function(){
          q.querySelectorAll('button').forEach(function(x){x.classList.remove('on');});
          b.classList.add('on');
          ans[q.getAttribute('data-q')]=b.getAttribute('data-v');
          if(Object.keys(ans).length===3){
            var good=ans['0']==='ja'&&ans['1']==='ja'&&ans['2']==='nein';
            document.getElementById('sq-ok').style.display=good?'block':'none';
            document.getElementById('sq-talk').style.display=good?'none':'block';
          }
        });
      });
    });
  ...
  /* Bewertungs-Dots */
  var scroller=document.querySelector('.m-review-scroll'),dots=document.getElementById('review-dots');
  if(scroller&&dots){
    var items=scroller.querySelectorAll('.m-review-card'),ds=dots.querySelectorAll('i');
    scroller.addEventListener('scroll',function(){
      var i=Math.round(scroller.scrollLeft/(scroller.scrollWidth/items.length));
  ...
  if(chips){
    chips.querySelectorAll('button').forEach(function(b){
      b.addEventListener('click',function(){
        ...
        document.getElementById('c_rueckruf').value=on?'':b.getAttribute('data-v');
      });
    });
  }
  /* Rest-Minuten im Wizard */
  var cur=document.getElementById('cur-step'),eta=document.getElementById('wz-eta');
  if(cur&&eta&&window.MutationObserver){
    var upd=function(){var s=parseInt(cur.textContent,10)||1;eta.textContent=Math.max(1,Math.round((6-s)*1.6));};
    new MutationObserver(upd).observe(cur,{childList:true,characterData:true,subtree:true});
    upd();
  }
  /* Exit-Anker */
  var bar=document.getElementById('exit-bar');
  if(bar){
    var lastY=0,shown=false,dismissed=sessionStorage.getItem('so-exitbar')==='1';
    document.getElementById('exit-bar-close').addEventListener('click',function(){
      bar.classList.remove('show');dismissed=true;sessionStorage.setItem('so-exitbar','1');
    });
    bar.querySelector('a').addEventListener('click',function(){bar.classList.remove('show');});
    window.addEventListener('scroll',function(){
      var y=window.scrollY,startView=!location.hash||location.hash==='#start';
      if(!dismissed&&startView&&y<lastY-60&&y>800&&!shown){bar.hidden=false;requestAnimationFrame(function(){bar.classList.add('show');shown=true;});}
      if(y<80&&shown){bar.classList.remove('show');}
      lastY=y;
    },{passive:true});
    window.addEventListener('hashchange',function(){bar.classList.remove('show');});
  }
})();
